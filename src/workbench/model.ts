...
export function exportSvg(layout:Layout) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${layout.width}mm" height="${layout.height}mm" viewBox="0 0 ${layout.width} ${layout.height}"><title>ScrollWorks motif layout</title><g fill="none" stroke="#000" stroke-linecap="round" stroke-linejoin="round">${layout.printBackbone?`<path d="${curvePath(layout.curve)}" stroke-width="0.35"/>`:''}${layout.items.map(i=>{const p=placedPaths(layout,i);return `<g><path d="${p.outline}" stroke-width="0.35"/>${p.folds?`<path d="${p.folds}" stroke-width="0.2"/>`:''}</g>`;}).join('')}</g></svg>`;
}
